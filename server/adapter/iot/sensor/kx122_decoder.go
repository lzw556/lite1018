...
func (s Kx122Decoder) Decode(data []byte, metaLength int) (map[string]interface{}, error) {
	// decode metadata
	...
	xlog.Infof("receive metadata %s", fmt.Sprintf("%+v", metadata))
	...
	if len(valueBytes) == totalLength {
		...
		for _, m := range metadata {
			...
			case 6: // uint24
				svtRawData.SetValues(int(m.axis), decodeUint24(axisData, m.offset, m.k))
			}
...
func decodeInt16(data []byte, offset float32, k float32) []float64 {
	j := 0
	values := make([]float64, 0)
	for j < len(data) {
		var value = int16(binary.LittleEndian.Uint16(data[j : j+2]))
		if k > 0 {
			values = append(values, (float64(value)-float64(offset))*float64(k))
		} else {
...
func decodeUint24(data []byte, offset float32, k float32) []float64 {
	values := make([]float64, 0)
