...
func NewCalibrateResponse() CalibrateResponse {
	return CalibrateResponse{}
}

func (d CalibrateResponse) Name() string {
	return "calibrateResponse"
}

func (d CalibrateResponse) Dispatch(msg iot.Message) {
	...
	if _, err := cache.Get(m.ReqId); err == nil {
		xlog.Infof("[%s] calibrate command executed successful req id %s", msg.Body.Device, m.ReqId)
