...
func NewAddDeviceResponse() AddDeviceResponse {
	return AddDeviceResponse{}
}

func (d AddDeviceResponse) Name() string {
	return "addDeviceResponse"
}

func (d AddDeviceResponse) Dispatch(msg iot.Message) {
	...
	if _, err := cache.Get(m.ReqId); err == nil {
		xlog.Infof("[%s] add device command executed successful req id %s", msg.Body.Device, m.ReqId)
