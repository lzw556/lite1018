...
type UpdateWsnResponse struct {
}

func NewUpdateWsnResponse() UpdateWsnResponse {
	return UpdateWsnResponse{}
}

func (d UpdateWsnResponse) Name() string {
	return "updateWsnResponse"
}

func (d UpdateWsnResponse) Dispatch(msg iot.Message) {
	...
	if _, err := cache.Get(m.ReqId); err == nil {
		xlog.Infof("[%s] updateWsn command executed successful req id %s", msg.Body.Device, m.ReqId)
