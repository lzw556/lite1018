...
const AllocUserDrawer: FC<AllocUserDrawerProps> = (props) => {
    ...
    const onChange = (value: any) => {
        setTreeData(convertTreeData(dataSource?.filter(item => item.user.username.indexOf(value) !== -1)));
        setCheckedKeys(convertCheckedKeys(dataSource?.filter(item => item.user.username.indexOf(value) !== -1)));
    }

    return <Drawer {...props} title={project.name} placement={"right"} extra={renderExtra()}>
        <Row justify={"center"}>
            <Col span={24}>
        ...
        <Row justify={"start"}>
            <Col span={24}>
                {
                    treeData && <Tree defaultExpandAll={true} treeData={treeData}
                                      showIcon={true} selectable={false}
                                      checkable={true} checkedKeys={checkedKeys}
                                      onCheck={onCheck}/>
                }
            </Col>
        </Row>
