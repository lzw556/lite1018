...
const DeviceList: FC<DeviceTableProps> = ({ network, onRefresh }) => {
  ...
  const onDelete = (id: number) => {
    RemoveDevicesRequest(network.id, { device_ids: [id] }).then((_) =>
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: string, record: Device) => {
        return (
          <Space>
            {
              <a href={`#/device-management?locale=networks/deviceDetail&id=${record.id}`}>
                {text}
              </a>
            }
    ...
    },
    {
      ...
      key: 'macAddress',
      render: (text: string) => {
        return <Typography.Text>{text.toUpperCase().macSeparator()}</Typography.Text>;
      }
    },
    {
      title: '设备类型',
          ...
          <Space>
            {hasPermission(Permission.NetworkRemoveDevices) && record.id !== network.gateway.id && (
              <Popconfirm
                ...
                cancelText='取消'
              >
                <Button type='text' size='small' danger disabled={isUpgrading}>
                  移除
                </Button>
              </Popconfirm>
            )}
  ...
  return (
    <>
      <DeviceTable
        columns={columns}
        permissions={[Permission.NetworkRemoveDevices]}
