...
const RuleFormItem: FC<RuleFormItemProps> = ({ fields, defaultValue }) => {
  ...
  return (
    <>
      ...
      <Row justify={'start'}>
        <Col span={7}>
          <Form.Item required label={'阈值条件'} name={['rule', 'operation']} initialValue={'>'}>
            <Select size={'middle'} defaultActiveFirstOption={true} style={{ width: '64px' }}>
              <Option key={'>'} value={'>'}>
                &gt;
              </Option>
              <Option key={'>='} value={'>='}>
        ...
        <Col span={5}>
          <Form.Item
            name={['rule', 'threshold']}
            normalize={Normalizes.float}
            rules={[Rules.number]}
          >
            <Input placeholder={'报警阈值'} size={'middle'} suffix={field?.unit} />
          </Form.Item>
        </Col>
      </Row>
    </>
