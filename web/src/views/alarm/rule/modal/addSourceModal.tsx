        ...
        <Form form={form} labelCol={{span: 4}} validateMessages={defaultValidateMessages}>
            {
                ...
                <Form.Item label={"请选择网络"}>
                    <Select placeholder={"请选择网络"} onDropdownVisibleChange={onFilter} onChange={value => {
                        setNetwork(networks.find((item:any) => item.id === value))
                    }}>
                        {
                            networks.map((item:any) => <Option key={item.id} value={item.id}>{item.name}</Option>)
                        }
                    </Select>
                </Form.Item>
            }
            <Form.Item label={"监控对象"} name={"ids"}>
                <Select placeholder={"请选择监控对象"} mode={"multiple"} maxTagCount={4} dropdownRender={menu => {
                    return <div>
                        {menu}
                    ...
                    {sources.map((source: any) =>
                        <Option key={source.id} value={source.id}>
                            ...
                            <Typography.Text
                                type={"secondary"}>{source.macAddress.macSeparator().toUpperCase()}</Typography.Text>
                        </Option>)}
                </Select>
            </Form.Item>
        </Form>
