  ...
  const columns: any = [
    ...
    },
    {
      ...
      render: (enabled: boolean) => {
        return enabled ? (
          <Typography.Text strong style={{ color: ColorHealth }}>
            启用
          </Typography.Text>
        ) : (
          <Typography.Text strong style={{ color: ColorDanger }}>
            停用
          </Typography.Text>
        );
      }
        ...
        return (
          <>
            <HasPermission value={Permission.AlarmRuleStatusEdit}>
              {record.enabled ? (
                <a style={{ color: ColorDanger }} onClick={() => onEditStatus(record.id, 0)}>
                  停用
                </a>
              ) : (
                <a style={{ color: ColorHealth }} onClick={() => onEditStatus(record.id, 1)}>
                  启用
                </a>
              )}
            ...
            <Dropdown overlay={renderEditMenus(record)}>
              <Button
                ...
                hidden={
                  !(
                    hasPermission(Permission.AlarmRuleEdit) ||
                    hasPermission(Permission.AlarmSourceAdd)
                  )
                }
              />
            </Dropdown>
        ...
        },
        {
          ...
          render: (_: any, source: any) => {
            return (
              <Space>
                <Popconfirm
                  title={`确认要将【${source.name}】移除【${record.name}】报警规则吗?`}
                  onConfirm={() => onRemoveDeviceFromAlarmRule(record.id, source.id)}
                  okText='移除'
                  cancelText='取消'
                >
                  <Button type='text' size='small' danger>
      ...
      <Row justify={'start'}>
        <Col span={24}>
          <TableLayout
            ...
            columns={columns}
            permissions={[
              Permission.AlarmRuleStatusEdit,
              Permission.AlarmRuleEdit,
              Permission.AlarmRuleDelete
            ]}
            expandable={{
