...
import {Col, Popover, Row, Statistic, Tag} from 'antd';
...
export const SingleDeviceInfo: React.FC<{ device: Device; actions?: React.ReactNode[] }> = ({
    ...
    const {
        state: {isOnline, batteryVoltage, signalLevel},
        name,
        typeId,
        macAddress,
        data: {values},
    ...
    const Title = () => {
        ...
        return (
            <>
                {isOnline ? <Tag color={ColorHealth}>在线</Tag> : <Tag color={ColorWarn}>离线</Tag>}
                {deviceName}
            </>
        );
    };
    const AlertState = () => {
        if (alertStates.length === 0) return null;
