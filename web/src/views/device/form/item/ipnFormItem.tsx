...
const IpnFormItem:FC<IpnFromItemProps> = ({ipn}) => {
    const [isDhcpEnabled, setIsDhcpEnabled] = useState<boolean>(ipn.ip_mode === 0)
    const [isNtpEnabled, setIsNtpEnabled] = useState<boolean>(ipn.ntp_is_enabled)

    const renderIPFormItem = () => {
        if (!isDhcpEnabled) {
            return <div>
                <Form.Item label={"IP地址"} name="ip_addr" initialValue={ipn.ip_addr}
                           rules={[{required: true, message: "请输入IP地址"}]}>
                    <Input placeholder={"请输入网关IP地址"}/>
                </Form.Item>
                <Form.Item label={"子网掩码"} name="subnet_mask" initialValue={ipn.subnet_mask}
                           rules={[{required: true, message: "请输入子网掩码"}]}>
                    <Input placeholder={"请输入网关子网掩码"}/>
                </Form.Item>
                <Form.Item label={"网关地址"} name="gateway_addr" initialValue={ipn.gateway_addr}
    ...
    const renderNTPFormItem = () => {
        if (isNtpEnabled) {
            return <Form.Item label={"NTP地址"} initialValue={ipn.ntp_addr} name={"ntp_addr"}
    ...
    return <div>
        <Form.Item label={"IP模式"} required name="ip_mode" initialValue={ipn.ip_mode}>
            <Radio.Group value={ipn.ip_mode} onChange={(e) => {
        ...
        <Form.Item label={"是否启用NTP"} required name="ntp_is_enabled" initialValue={ipn.ntp_is_enabled}>
            <Radio.Group value={ipn.ntp_is_enabled} onChange={(e) => {
