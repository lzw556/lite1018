...
const InformationCard: FC<GatewayInformationProps> = ({device}) => {
    ...
    return <ShadowCard>
        ...
        <Row justify={"start"}>
            ...
            <Col span={6}>
                {
                    device.status && device.status.isOnline ? (<Tag color={ColorHealth}>在线</Tag>) : (<Tag color={ColorWarn}>离线</Tag>)
                }
            </Col>
            <Col span={3} className="ts-detail-label">
                所属资产
            ...
            <Col span={6} hidden={device.typeId !== DeviceType.Gateway}>
                {
                    device.information.ip_address ?
                        (<Space>{device.information.ip_address} <a href={`http://${device.information.ip_address}`} target={"_blank"} style={{fontSize: "10pt"}}>访问管理界面</a></Space>) :
                        "-"
                }
            </Col>
        </Row>
