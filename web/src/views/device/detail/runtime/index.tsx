...
const RuntimeData: FC<RuntimeDataProps> = ({ device }) => {
  ...
  const fetchRuntimeData = useCallback(() => {
    GetDeviceRuntimeRequest(device.id, startDate.utc().unix(), endDate.utc().unix()).then(
      (data) => {
        const batteryOption = {
  ...
  const renderCharts = () => {
    if (options && options.length) {
      return options.map((item: any, index: number) => {
        return (
          <Card.Grid key={index} style={{ boxShadow: 'none', border: 'none' }}>
  ...
  return (
    <div>
      ...
      <Row justify={'start'} style={{ paddingTop: '4px' }}>
        <Col span={24}>
          <Card bordered={false}>{renderCharts()}</Card>
        </Col>
      </Row>
    </div>
  );
