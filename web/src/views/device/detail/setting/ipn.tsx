...
const IPNSetting: FC<IPNSettingProps> = ({values, device}) => {
    ...
    const onRefreshSetting = (setting: any) => {
        setIPN(Object.assign({}, ipn, setting))
    }

    return <Card bordered={false} size={"small"}>
        <Row justify={"start"}>
            <Col span={9}>
                <Setting device={device} name={"ip_mode"} label={"IP模式"}
                         value={ipn.ip_mode}
                         tooltip={"建议设为DHCP模式,如果设为静态模式则需要同时配置IP地址,子网掩码和网关地址"}
                         displayRender={item => {
                             return item ? "静态" : "DHCP"
                         }}
                         editable={item => {
            ...
            <Col span={9} hidden={ipn.ip_mode === 0}>
                <Setting device={device} name={"ip_addr"} label={"IP地址"}
                         value={ipn.ip_addr}
                         editable={item => {
        ...
        <Row justify={"start"} hidden={ipn.ip_mode === 0}>
            <Col span={9}>
                <Setting device={device} name={"subnet_mask"} label={"子网掩码"}
                         value={ipn.subnet_mask}
                         editable={item => {
            ...
            <Col span={9}>
                <Setting device={device} name={"gateway_addr"} label={"网关地址"}
                         value={ipn.gateway_addr}
                         editable={item => {
        ...
        <Row justify={"start"}>
            <Col span={9}>
                <Setting device={device} name={"ntp_is_enabled"} label={"NTP是否启用"}
                         value={ipn.ntp_is_enabled}
                         tooltip={"建议当网关处于局域网时禁用,如果启用则需要配置NTP地址"}
                         displayRender={item => item ? "启用" : "禁用"}
                         editable={item => {
                             return <Select defaultValue={ipn.ntp_is_enabled ? 1 : 0} size={"small"}
                                            onChange={value => {
            ...
            <Col span={9} hidden={!ipn.ntp_is_enabled}>
                <Setting device={device} name={"ntp_addr"} label={"NTP地址"}
                         value={ipn.ntp_addr}
                         tooltip={"网关进行时间同步的地址,默认地址: ntp1.aliyun.com"}
                         editable={item => {
