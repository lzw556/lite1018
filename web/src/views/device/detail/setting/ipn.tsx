...
const IPNSetting: FC<IPNSettingProps> = ({values, device}) => {
    ...
    return <Card bordered={false} size={"small"}>
        <Row justify={"start"}>
            <Col span={9}>
                <Setting device={device} name={"ip_mode"} label={"IP模式"} value={ipn.ip_mode} tooltip={"建议设为DHCP模式,如果设为静态模式则需要同时配置IP地址,子网掩码和网关地址"} renderValue={item => {
                    return item ? "静态" : "DHCP"
            ...
            <Col span={9}>
                <Setting device={device} name={"ip_addr"} label={"IP地址"} value={ipn.ip_addr} editable={ipn.ip_mode === 1} renderEdit={item => {
        ...
        <Row justify={"start"}>
            <Col span={9}>
                <Setting device={device} value={ipn.subnet_mask} name={"subnet_mask"} label={"子网掩码"} editable={ipn.ip_mode === 1} renderEdit={item => {
            ...
            <Col span={9}>
                <Setting device={device} value={ipn.gateway_addr} name={"gateway_addr"} label={"网关地址"} editable={ipn.ip_mode === 1} renderEdit={item => {
        ...
        <Row justify={"start"}>
            <Col span={9}>
                <Setting device={device} value={ipn.ntp_is_enabled} name={"ntp_is_enabled"} label={"NTP是否启用"} editable={true} renderValue={(item => {
                    return item ? "启用" : "禁用"
                })} renderEdit={item => {
                ...
                }} onSuccess={onRefreshSetting} tooltip={"建议当网关处于局域网时禁用,如果启用则需要配置NTP地址"}/>
            </Col>
            <Col span={9}>
                <Setting device={device} value={ipn.ntp_addr} name={"ntp_addr"} label={"NTP地址"} editable={ipn.ntp_is_enabled} renderEdit={item => {
                    return <Input size={"small"} defaultValue={item.ntp_addr} onChange={e => {
                        item.ntp_addr = e.target.value
                    }}/>
                }} onSuccess={onRefreshSetting} tooltip={"网关进行时间同步的地址,默认地址: ntp1.aliyun.com"}/>
            </Col>
        </Row>
    </Card>
}
