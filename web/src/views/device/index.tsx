...
import {generateDeviceTypeCollections, getValueOfFirstClassProperty} from './util';
    ...
    const renderEditMenus = (record: Device) => {
        ...
        return (
            ...
            >
                ...
                {record.typeId !== DeviceType.Gateway && record.typeId !== DeviceType.Router &&
                    <Menu.Item key={3}>编辑报警规则</Menu.Item>}
            </Menu>
        );
    };
                        ...
                        <Space direction={isMobile ? 'vertical' : 'horizontal'}>
                            <Label name={'网络'}>
                                <NetworkSelect bordered={false} onChange={(network) => {
                                    dispatch(setDeviceFilterAction({networkId: network}))
                                    setDeviceFilter({networkId: network})
                                }} allowClear defaultValue={deviceFilter.networkId > 0 ? deviceFilter.networkId : null}/>
                            </Label>
                            <Label name='设备类型'>
                                <Select
                                    placeholder='请选择设备类型'
                            ...
                            <Input.Group compact>
                                ...
                                <Search
                                    ...
                                    onSearch={val => {
                                        setFilter((prev: any) => {
                                            return searchTarget === 0 ? {...prev, name: val} : {
                                                ...prev,
                                                mac_address: val
                                            }
                                        })
                    ...
                    <Col span={24}>
                        <DeviceTable
                            ...
                            onChange={(index: number, size: number) => {
                                setPage({current: index, size: size});
                                dispatch(setPageAction({current: index, size: size}));
                            }}
                        />
                    </Col>
                </Row>
