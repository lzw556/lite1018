...
const DashboardPage = () => {
  ...
  const fetchAlarmStatistics = () => {
    GetAlarmRecordStatisticsRequest(
      moment().local().startOf('day').unix(),
      moment().local().endOf('day').unix(),
      {}
    ).then(setAlarmStatistics);
  };

  const renderAlarmStatistics = () => {
    const untreated = alarmStatistics
      ? alarmStatistics.untreated.reduce((acc: number, cur: number) => acc + cur, 0)
      : 0;
    const resolved = alarmStatistics
      ? alarmStatistics.resolved.reduce((acc: number, cur: number) => acc + cur, 0)
      : 0;
    const recovered = alarmStatistics
      ? alarmStatistics.recovered.reduce((acc: number, cur: number) => acc + cur, 0)
      : 0;
    ...
    <DashboardLayout>
      <Row justify={'start'} style={{ position: 'relative', height: '256x', width: '100%' }}>
        ...
        <Col span={6}>
          ...
          <Row justify={'start'}>
            <Col span={24}>{/*<AlarmRecordCard/>*/}</Col>
          </Row>
        </Col>
      </Row>
    </DashboardLayout>
