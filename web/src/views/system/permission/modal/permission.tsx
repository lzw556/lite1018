...
const PermissionModal: FC<PermissionModalProps> = (props) => {
    ...
    return <Modal {...props}>
        <Form form={form} labelCol={{span: 6}}>
            <Form.Item name="path" label={"权限路径"} rules={[{required: true, message: "请输入权限路径"}]}>
            ...
            <Form.Item name={"group"} label={"分组"} rules={[Rules.required]}>
                <Select placeholder={"请选择分组"}>
                    <Option key={"asset"} value={"资产模块"}>资产模块</Option>
                    <Option key={"device"} value={"设备模块"}>设备模块</Option>
                    <Option key={"network"} value={"网络模块"}>网络模块</Option>
                    <Option key={"user"} value={"用户模块"}>用户模块</Option>
                    <Option key={"alarm"} value={"报警模块"}>报警模块</Option>
                    <Option key={"data"} value={"data"}>数据模块</Option>
                </Select>
            </Form.Item>
        </Form>
    </Modal>
