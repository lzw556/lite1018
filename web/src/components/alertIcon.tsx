...
const AlertIcon: FC<AlertIconProps> = ({ state, popoverPlacement }) => {
  ...
  const onShow = (visible: boolean) => {
    if (visible) {
      ...
      GetAlarmRecordRequest(state.record.id)
        .then((data) => {
          setContent(
            <Space>
              ...
              <Text>{`【${GetFieldName(data.rule.field)}】值${OperationTranslate(
                data.rule.operation
              )}设定的阈值${data.rule.threshold}${data.property.unit}`}</Text>
            </Space>
          );
        })
        .catch((e) => {
  ...
  const renderAlertPopover = () => {
    if (state.level > 0) {
      ...
      return (
        <Popover placement={popoverPlacement} content={content} onVisibleChange={onShow}>
