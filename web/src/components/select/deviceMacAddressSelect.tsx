  ...
  return (
    <>
      <Select
        ...
        open={false}
        tagRender={(props) => (
          <Tag {...props} closable={true} onClose={(e) => onRemoveBindingDevice(props.label)}>
            {props.label}
          </Tag>
        )}
        ref={(select) => {
      ...
      />
      {
        <BindingDeviceModal
          visible={visible}
          quantity={
            type === MeasurementType.FlangeLoosening || type === MeasurementType.FlangeElongation
              ? settings.number_of_bolts
              : 1
          }
          onCancel={() => {
            setVisible(false);
