...
const LocationSelect: FC<LocationPickUpProps> = ({
  ...
  const renderImage = () => {
    if (status === 'loaded' && image) {
      ...
      return (
        <Group x={x} y={y} width={image.width * scale} height={image.height * scale}>
          <Image
          ...
          {point && (
            <Circle
              x={point.x * image.width * scale}
              y={point.y * image.height * scale}
              radius={5}
      ...
      >
        ...
        {point && (
          <Row justify={'center'} style={{ textAlign: 'center' }}>
            <Col span={12}>{`X轴坐标: ${point.x}`}</Col>
            <Col span={12}>{`Y轴坐标: ${point.y}`}</Col>
          </Row>
        )}
      </Modal>
    </>
