...
license:
ifeq ($(os),windows)
	cd server && CGO_ENABLED=1 GOOS=$(os) GOARCH=amd64 CC=x86_64-w64-mingw32-gcc go build --tags license -o ../deploy/script/windows/bin/cloud.exe ./main.go
endif
ifeq ($(os),linux)
	cd server && CGO_ENABLED=1 GOOS=linux GOARCH=amd64 CC=x86_64-linux-musl-gcc go build --tags license -ldflags "-linkmode external -extldflags -static" -o ../deploy/script/linux/bin/cloud-lite ./main.go
endif
ifeq ($(os),drawin)
