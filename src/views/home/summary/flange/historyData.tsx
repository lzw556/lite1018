...
export const HistoryData: React.FC<AssetRow> = (props) => {
  ...
  const [virtualPoint, setVirtualPoint] = React.useState<MeasurementRow | undefined>();

  React.useEffect(() => {
    setVirtualPoint(
      props.monitoringPoints?.find((point) => point.type === measurementTypes.flangePreload.id)
    );
